...
import com.sigurtest.model.Person;
...
public class PassEmulator {
    ...
    private void generateAccessLogMessage(Person person, LocalDate currentDate) {
        if (person.getClass().equals(Employee.class)) {
            ...
            String message = employee.getFiredTime() != null ? String.format("%s Доступ запрещён сотруднику %d. Отдел: %s. Карта: %s.",
                currentDate, employee.getId(), departmentName, cardHexString) : String.format("%s Предоставлен доступ сотруднику %d. Отдел: %s. Карта: %s.",
            ...
            String message = employeeToVisit.getFiredTime() == null ? String.format("%s Предоставлен доступ гостю %d. Пришёл к %d из отдела: %s. Карта: %s.",
                currentDate, guest.getId(), employeeToVisit.getId(), departmentName, guestCardHexString) : String.format("%s Доступ запрещён гостю %d. Карта: %s.",
